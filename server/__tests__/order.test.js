...
describe('api test', function(){
    test('signin', signInClient());
    test('login client', loginClient());
    ...
    test("create order", function(done){
        server
    ...
    test('signin manufacturer', signInManufacturer());
    test('login manufacturer', loginManufacturer());
    ...
    test("get order content", function(done){
        server
            .post("/api/order/getordercontent")
            .send({"batchId": 1})
            .expect([{
                    "partId": 1
            }])
...
function signInClient() {
    return function(done){
        server
            .post('/auth/signin')
            .send({"email": "testclient@hotmail.com", "passw": "testclientpassword", "userType": "client"})
            .expect("succesfully created user with email testclient@hotmail.com")
            .expect(200,done)
    }
}
...
function loginClient() {
    return function(done) {
        server
            .post('/auth/login')
            .send({"email": "testclient@hotmail.com", "passw": "testclientpassword"})
            .expect("welcome in")
            .expect(200,done)
    };
};
function signInManufacturer() {
    return function(done){
        server
            .post('/auth/signin')
            .send({"email": "testmanufacturer@hotmail.com", "passw": "testmanupassword", "userType": "manufacturer"})
            .expect("succesfully created user with email testmanufacturer@hotmail.com")
            .expect(200,done)
    }
}
...
function loginManufacturer() {
    return function(done) {
        server
            .post('/auth/login')
            .send({"email": "testmanufacturer@hotmail.com", "passw": "testmanupassword"})
            .expect("welcome in")
            .expect(200,done)
    };
};
